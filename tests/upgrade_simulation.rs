...
fn test_max_pnl_force_close_profitable_trader() {
    ...
    let outcome = engine
        .keeper_crank(u16::MAX, 10, ORACLE_1_3M, 0, false, 2000)
        .unwrap();
    ...
    let outcome2 = engine
        .keeper_crank(u16::MAX, 20, ORACLE_1_3M, 0, false, 1000)
        .unwrap();
...
fn test_max_pnl_does_not_close_lp() {
    ...
    let outcome = engine
        .keeper_crank(u16::MAX, 10, ORACLE_800K, 0, false, 500) // Very tight 5% cap
        .unwrap();
...
fn test_max_pnl_disabled_when_zero() {
    ...
    let outcome = engine
        .keeper_crank(u16::MAX, 10, ORACLE_1_3M, 0, false, 0)
        .unwrap();
...
fn test_max_pnl_selective_close_only_profitable() {
    ...
    let outcome = engine
        .keeper_crank(u16::MAX, 20, ORACLE_1_3M, 0, false, 1000) // 10% cap
        .unwrap();
...
fn test_vault_drain_protection_e2e() {
    ...
    let outcome = engine
        .keeper_crank(u16::MAX, 100, 1_500_000, 0, false, 2000)
        .unwrap();
...
fn test_crank_outcome_has_max_pnl_fields() {
    ...
    let outcome = engine
        .keeper_crank(u16::MAX, 1, ORACLE_1M, 0, false, 0)
        .unwrap();
...
fn test_progressive_pnl_growth_triggers_force_close() {
    ...
    for (i, &price) in prices.iter().enumerate() {
        ...
        let outcome = engine
            .keeper_crank(u16::MAX, slot, price, 0, false, 1500) // 15% cap
            .unwrap();
