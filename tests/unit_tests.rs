...
fn test_keeper_crank_liquidates_undercollateralized_user() {
    ...
    let result = engine.keeper_crank(user, 1, 500_000, 0, false, 0, 0);
    assert!(result.is_ok());
    ...
    for slot in 2..=17 {
        engine.keeper_crank(user, slot, 500_000, 0, false, 0, 0).unwrap();
    }
...
fn test_gc_fee_drained_dust() {
    ...
    let outcome = engine.keeper_crank(user, 10, 1_000_000, 0, false, 0, 0).unwrap();
...
fn test_gc_positive_pnl_never_collected() {
    ...
    let outcome = engine
        .keeper_crank(u16::MAX, 100, 1_000_000, 0, false, 0, 0)
        .unwrap();
...
fn test_batched_adl_profit_exclusion() {
    ...
    let outcome = engine
        .keeper_crank(u16::MAX, 1, crank_oracle, 0, false, 0, 0)
        .unwrap();
    ...
    for slot in 2..20 {
        engine
            .keeper_crank(u16::MAX, slot, crank_oracle, 0, false, 0, 0)
            .unwrap();
    }
...
fn test_batched_adl_conservation_basic() {
    ...
    let outcome = engine
        .keeper_crank(u16::MAX, 1, 1_000_000, 0, false, 0, 0)
        .unwrap();
...
fn test_two_phase_liquidation_priority_and_sweep() {
    ...
    while !engine.last_full_sweep_completed_slot > 0 && slot < 100 {
        let outcome = engine
            .keeper_crank(u16::MAX, slot, 1_000_000, 0, false, 0, 0)
            .unwrap();
...
fn test_pending_finalize_liveness_insurance_covers() {
    ...
    // Run enough cranks to complete a full sweep
    for slot in 1..=16 {
        let result = engine.keeper_crank(u16::MAX, slot, 1_000_000, 0, false, 0, 0);
        assert!(result.is_ok());
    }
...
fn test_force_realize_updates_lp_aggregates() {
    ...
    let result = engine.keeper_crank(u16::MAX, 1, 1_000_000, 0, false, 0, 0);
    assert!(result.is_ok());
...
fn test_idle_user_drains_and_gc_closes() {
    ...
    let outcome = engine
        .keeper_crank(user_idx, 1001, ORACLE_100K, 0, false, 0, 0)
        .unwrap();
...
fn test_dust_stale_funding_gc() {
    ...
    let outcome = engine
        .keeper_crank(user_idx, 10, ORACLE_100K, 0, false, 0, 0)
        .unwrap();
...
fn test_lp_never_gc() {
    ...
    for slot in 1..=10 {
        let outcome = engine
            .keeper_crank(lp_idx, slot * 100, ORACLE_100K, 0, false, 0, 0)
            .unwrap();
...
fn test_abandoned_with_stale_last_fee_slot_eventually_closed() {
    ...
    let _ = engine
        .keeper_crank(user_idx, 10_000, ORACLE_100K, 0, false, 0, 0)
        .unwrap();

    // Second crank: GC scan should pick up the dust
    let _outcome = engine
        .keeper_crank(user_idx, 10_001, ORACLE_100K, 0, false, 0, 0)
        .unwrap();
